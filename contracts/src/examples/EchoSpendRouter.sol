...
///         CREATE2 view (e.g. an escrow vault's claim-bound deposit view).
///         Lives under src/examples/ so Foundry compiles it, but consumers
///         importing the lib simply never import it.
///
/// @dev ⚠ TEST-ONLY — NEVER DEPLOY WITH A REAL TOKEN. The hash-derived
///      destination has no code, no key, and no refund path: tokens sent
///      there are UNRECOVERABLE. Echo demonstrates the beneficiary-BINDING
///      half of the adoption contract only. A real consumer's destination
///      must ALSO be recoverable by the beneficiary (e.g. a vault's
///      permissionless claim-bound refund, pinned by a refund round-trip
///      test in the consumer's own suite). Recoverability is a hard
///      adoption rule, not a suggestion.
