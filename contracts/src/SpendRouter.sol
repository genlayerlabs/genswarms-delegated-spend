...
abstract contract SpendRouter is ISpendRouter {
    ...
    ///      never replay it. The reentrancy lock rejects ANY nested spend (a
    ///      reentrant token interleaving a second spend at another destination
    ///      would otherwise slip past the exact-delivery check — with the
    ///      pinned USDC this is unreachable, but the lock makes the accounting
    ///      token-independent). The exact-delivery check rejects
    ///      fee-on-transfer behavior (destination must receive exactly
    ///      `amount`).
