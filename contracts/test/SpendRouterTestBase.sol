...
abstract contract SpendRouterTestBase is Test {
    ...
    function test_permit_expired_but_allowance_sufficient_proceeds() public {
        // Pinned by design: the front-run-tolerance branch checks ALLOWANCE,
        // not the permit deadline. An expired/garbage permit rides a
        // sufficient standing allowance — funds still land user-bound, and
        // order freshness (TTL) is enforced by the keeper's
        // server-authoritative order, not by the permit deadline.
        vm.prank(user);
        token.approve(address(_router()), 25e6);
        _executeWithPermit(
            keeper,
            userPk,
            25e6,
            keccak256("srtb-order-10"),
            block.timestamp - 1,
            27,
            bytes32(0),
            bytes32(0)
    ...
    function test_permit_allowance_one_below_rejects() public {
        // Boundary pin for the front-run-tolerance branch: `allowance < value`
        // must reject at exactly one unit short. A garbage permit forces the
        // catch branch, and the standing allowance of value-1 is insufficient.
        _approveAs(user, 25e6 - 1);
        vm.expectRevert(SpendRouter.PermitRejected.selector);
        _executeWithPermit(
            keeper,
            userPk,
            25e6,
            keccak256("srtb-order-11"),
    ...
    // ── fuzzed conservation, both lanes ─────────────────────────────────────
    function testFuzz_direct_conservation(uint256 amountSeed, bytes32 orderId) public {
        vm.assume(orderId != bytes32(0));
        uint256 amount = bound(amountSeed, 1, 1_000_000e6);
        address dest = _expectedDestination(user);
        uint256 u0 = token.balanceOf(user);
        uint256 d0 = token.balanceOf(dest);
        _approveAs(user, amount);
        _executeAs(user, amount, orderId);
        assertEq(token.balanceOf(user), u0 - amount);
        assertEq(token.balanceOf(dest), d0 + amount);
        assertEq(token.balanceOf(address(_router())), 0);
        assertTrue(_router().orderConsumed(orderId));
    }

    function testFuzz_permit_conservation(uint256 amountSeed, bytes32 orderId) public {
        vm.assume(orderId != bytes32(0));
        uint256 amount = bound(amountSeed, 1, 1_000_000e6);
        uint256 deadline = block.timestamp + 1 hours;
        (uint8 v, bytes32 r, bytes32 s) = _signPermit(userPk, address(_router()), amount, deadline);
        address dest = _expectedDestination(user);
        uint256 u0 = token.balanceOf(user);
        uint256 d0 = token.balanceOf(dest);
        _executeWithPermit(keeper, userPk, amount, orderId, deadline, v, r, s);
        assertEq(token.balanceOf(user), u0 - amount);
        assertEq(token.balanceOf(dest), d0 + amount);
        assertEq(token.balanceOf(address(_router())), 0);
    }
